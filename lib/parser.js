...
module.exports = class Parser {
  ...
  */
  ...
  conditional_statement() {
    ...
    if (this.eat(Lexer.TokenTypes.IF)) {
      testExpression = this.boolean_expr();
      if (testExpression == null) {
        console.log(`Error processing IF..THEN..ELSE: Expected a boolean expression`);
        return null;
      }
    } else {
      console.log(`Error processing IF..THEN..ELSE: Expected "IF" got ${this.currentToken.type}`);
      return null;
    }

    if (this.eat(Lexer.TokenTypes.THEN)) {
      trueStatement = this.statement();
      if (trueStatement == null) {
        console.log(`Error processing IF..THEN..ELSE: Expected a statement following THEN`);
      }
    } else {
      console.log(`Error processing IF..THEN..ELSE: Expected "THEN" got ${this.currentToken.type}`);
      return null;
    }

    if (this.eat(Lexer.TokenTypes.ELSE)) {
  ...
  whiledo_statement() {
    ...
    if (!this.eat(Lexer.TokenTypes.WHILE)) {
      console.log(`Error processing WHILE..DO: Expected "WHILE" got ${this.currentToken.type}`);
      return null;
    }

    testExpression = this.boolean_expr();
    if (testExpression == null) {
      console.log(`Error processing WHILE..DO: Expected a boolean expression`);
      return null;
    }

    if (!this.eat(Lexer.TokenTypes.DO)) {
      console.log(`Error processing WHILE..DO: Expected "DO" got ${this.currentToken.type}`);
      return null;
    }

    loopStatement = this.statement();
    if (loopStatement == null) {
      console.log(`Error processing WHILE..DO: Expected a statement`);
      return null;
    }
  ...
  repeatuntil_statement() {
    ...
    if(!this.eat(Lexer.TokenTypes.REPEAT)) {
      console.log(`Error processing REPEAT..UNTIL: Expected "REPEAT" got ${this.currentToken.type}`);
      return null;
    }

    loopStatement = this.statement();
    if (loopStatement == null) {
      console.log(`Error processing REPEAT..UNTIL: Expected a statement`);
      return null;
    }

    if(!this.eat(Lexer.TokenTypes.UNTIL)) {
      console.log(`Error processing REPEAT..UNTIL: Expected "UNTIL" got ${this.currentToken.type}`);
      return null;
    }

    testExpression = this.boolean_expr();
    if (testExpression == null) {
      console.log(`Error processing REPEAT..UNTIL: Expected a boolean expression`);
      return null;
    }
  ...
  type_spec() {
    ...
    if (this.eat(Lexer.TokenTypes.TYPE_INTEGER) || this.eat(Lexer.TokenTypes.TYPE_REAL) || this.eat(Lexer.TokenTypes.TYPE_BOOLEAN)) {
  ...
  function_declaration() {
    let idToken = this.currentToken;
    if (!this.eat(Lexer.TokenTypes.ID)) {
      console.log(`Error processing FUNCTION: Expected an ID, got ${this.currentToken.type}`);
      return null;
    }

    let parameters = this.formal_parameter_declaration();
    if (parameters === null) {
      console.log(`Error processing FUNCTION: Expected a parameter declaration`);
      return null;
    }

    if (!this.eat(Lexer.TokenTypes.COLON)) {
      console.log(`Error processing FUNCTION: Expected a COLON, got ${this.currentToken.type}`);
      return null;
    }

    let typeSpec = this.type_spec();
    if (typeSpec === null) {
      console.log(`Error processing FUNCTION: Expected a typespec`);
      return null;
    }

    if (!this.eat(Lexer.TokenTypes.SEMI)) {
      console.log(`Error processing FUNCTION: Expected a SEMI, got ${this.currentToken.type}`);
      return null;
    }

    let block = this.block();
    if (block === null) {
      console.log(`Error processing FUNCTION: Expected a valid block`);
      return null;
    }
