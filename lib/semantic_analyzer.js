...
class SemanticAnalyzer {
  ...
  visit_PROCEDURE(node) {
    const procedure_symbol = new SymbolTable.ProcedureSymbol(node.name, node.block);
  ...
  visit_PROCEDURECALL(node) {
    ...
    if (isValid) {
      const procedureCall_symbol = new SymbolTable.ProcedureCallSymbol(node.name, node.params);

      // validate all params
      isValid = node.params.reduce((valid, paramNode) => { return valid && this.visit(paramNode) }, true);

      if (isValid) {
        this.symbolTable.define(procedureCall_symbol);
        return true;
      } else {
        return false;
      }
