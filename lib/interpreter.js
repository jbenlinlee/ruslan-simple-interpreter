...
module.exports = class Interpreter {
  ...
  visit_FUNCTIONCALL(node) {
    const functionSymbol = this.symbolTable_current.lookup(node.name);
    const functionFrame = new CallStack.CallStackFrame(this.currentStackFrame, functionSymbol.symbolTable);

    // Add function name to stack frame since Pascal has return vals assigned
    // to same name
    functionFrame.define(node.name);

    // Push frame onto call stack
    this.pushStackFrame(functionFrame);

    // Resolve parameter values and setup frame
    for (let i = 0; i < functionSymbol.params.length; ++i) {
      const paramName = functionSymbol.params[i].name;
      const paramVal = this.evalTree(node.params[i]);
      functionFrame.set(paramName, paramVal);
    }

    // Execute block and then return the return val
    this.visit_BLOCK(functionSymbol.block);
    const retVal = functionFrame.lookup(node.name);
    this.popStackFrame();
    return retVal;
  }

  visit_CONDITIONAL(node) {
    if (this.evalTree(node.test)) {
      this.evalTree(node.trueBlock);
    } else if (node.elseBlock) {
      this.evalTree(node.elseBlock);
    }
  }

  visit_WHILEDO(node) {
    while (this.evalTree(node.test)) {
      this.evalTree(node.body);
    }
  }
  ...
  visit_REPEATUNTIL(node) {
    do {
      this.evalTree(node.body);
    } while (!this.evalTree(node.test)); // Invert the test since the test specifies a stopping condition
  }
